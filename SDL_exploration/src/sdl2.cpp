...
class Grid {
   ...
   private:
    ...
    bool initGrid() {
        ...
        if (SDL_CreateWindowAndRenderer(m_pixel_width, m_pixel_height, 0,
        ...
        while (!quit) {
            ...
            while (SDL_PollEvent(&event)) {
                switch (event.type) {
                    ...
                    case SDL_MOUSEBUTTONDOWN:
                        ...
                        m_cursor.x =
                            (event.motion.x / m_cell_size) * m_cell_size;
                        m_cursor.y =
                            (event.motion.y / m_cell_size) * m_cell_size;
                        break;
                    case SDL_MOUSEMOTION:
                        // Set the position of the ghost cursor, when you are
                        // not clicking
                        m_ghost_cursor.x =
                            (event.motion.x / m_cell_size) * m_cell_size;
                        m_ghost_cursor.y =
            ...
            if (mouse_active && mouse_hover) {
                SDL_SetRenderDrawColor(
                    m_renderer, m_ghost_cursor_color.r, m_ghost_cursor_color.g,
                    m_ghost_cursor_color.b, m_ghost_cursor_color.a);
                SDL_RenderFillRect(m_renderer, &m_ghost_cursor);
            }

            // Draw the grid cursor
            SDL_SetRenderDrawColor(m_renderer, m_cursor_color.r,
                                   m_cursor_color.g, m_cursor_color.b,
                                   m_cursor_color.a);
